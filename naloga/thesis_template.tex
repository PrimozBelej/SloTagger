...
Računskega oblikoskladenjskega označevanja se lotevamo predvsem z algoritmi strojnega učenja. Za označevanje so se za učinkovite izkazali označevalniki temelječi na globokih nevronskih mrežah. Tak je tudi naš označevalnik, ki ga bomo predstavili v nadaljevanju.
...
Oblikoskladenjske oznake se uporabljajo kot dodatne značilke pri algoritmičnem reševanju nalog kot sta:
\begin{description}
...
Takšna predstavitev spremeni pristop k problemu. V primeru, da bi razrede predstavili s kodiranjem one-hot, bi oblikoskladenjsko označevanje reševali z večrazredno klasifikacijo, kjer bi bila vsaka možna oznaka en razred. V našem primeru imamo drugačen cilj, saj ne dodeljujemo enega razreda, temveč več različnih oznak. Takšen pristop imenujemo večznačna klasifikacija. Sprememba pristopa ima to slabost, da lahko model napoveduje kombinacije oblikoskladenjskih lastnosti, ki ne sodijo skupaj in tako tvori neveljavne oblikoskladenjske oznake. Primer neveljavne oznake je glagol, ki mu model pripiše sklon.
...
Za določanje oznak smo kot zadnji gradnik našega modela dodali sloj tesno povezanih enot. Teh enot je 118; toliko, kolikor je dolžina naših predstavitev oznak. Ker smo želeli v vsaki izmed enot dobiti oceno verjetnosti oznake smo za aktivacijo uporabili logistično funkcijo.
...
\subsection*{Mere kvalitete razvrščanja}

% predstavitev mer; kaj so mere; slabosti; prednosti
...
\subsubsection*{Priklic}

\subsubsection*{Mera F1}
...
% neenakomerna porazdelitev oznaka - slabost, ki se ji ne moremo izogniti

% za besedne vrste lahko razširjena ocena (f1, precision, recall)

% za celotne oznake samo klasifikacijska točnost in recall. precission ima tezave, ce za doloceno oznako ni napovedane vrednosti. recall ima tezave, ce za doloceno oznako ni resnicne vrednosti.

\begin{table}
\caption{Skupne ocene označevalnikov pri določanju besednih vrst. Pri merah preciznosti, priklica in F1 smo ocene po razredih utežili s številom vzorcev v razredu.}
    \begin{center}
        \begin{tabular}{l l l l l}
        \hline
Mera & Obeliks & Reldi & Nevro & Ansambel \\ \hline
Točnost & 0.983 & 0.988 & 0.991 & 0.992 \\
Preciznost & 0.983 & 0.988 & 0.991 & 0.992 \\
Priklic & 0.983 & 0.988 & 0.991 & 0.992 \\
Mera F1 & 0.983 & 0.988 & 0.991 & 0.992 \\\hline
        \end{tabular}
    \end{center}
\end{table}


% Najboljša ocena vsake vrstice naj bo odebeljena
\begin{table}
\caption{Ocene označevalnikov pri določanju besedne vrste. Ocene so izračunane iz mere F1 z desetkratnim prečnim preverjanjem. Skupna ocena označevalnikov je izračunana kot uteženo povprečje.}
    \begin{center}
        \begin{tabular}{l l l l l}
        \hline
...
0.99384178, 0.99561778, 0.98365072, 0.96087752, 0.99316284,
       0.99731259, 0.99873979, 0.99121025, 0.98785534, 0.75776335,
       0.99847572, 0.65402899, 0.9999939
...
2017-11-29\bibliographystyle{./bibliography/elsarticle-num}
